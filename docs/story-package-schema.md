...
## `pages.yaml`
- Defines how the story text is split across pages (or spreads).
...
| `license` | Optional | string | Rights/license label for internal governance. |
...
`pages.yaml` is the canonical pagination document. It maps story content into ordered display units.

### Goals
- Keep page sequencing explicit.
- Preserve stable page IDs for image and audio references.
...
| `pages` | Required | list[object] | Ordered page entries. |
| `pages[].id` | Required | string | Stable ID (e.g., `p01`) used by prompts/audio references. |
| `pages[].page_number` | Required | integer | Human-facing sequence number. |
| `pages[].text` | Required | string | Text shown/read for the page. |
| `pages[].illustration_focus` | Optional | string | Art-direction intent for illustrators/prompt authors. |
| `pages[].notes` | Optional | string | Editorial or layout comments specific to page. |
...
- Tie prompts directly to page IDs.
...
| `prompts[].page_id` | Required | string | Must match `pages[].id`. |
...
- Support **page-level clips** aligned to `pages[].id` (e.g., `page-01.wav`).
